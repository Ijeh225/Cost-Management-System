...
function StageColumn({
  ...
  return (
    <section data-stage={stage.value} aria-label={stage.label} className="flex min-w-0 flex-col w-full lg:w-56 shrink-0">
      ...
      <div className="space-y-2 lg:flex-1 lg:overflow-y-auto lg:pr-1 lg:max-h-[calc(100vh-220px)]">
        {containers.length === 0 ? (
...
export default function OperationsPage() {
  ...
  const jumpToStage = (stageValue: string) => {
    ...
    if (container) {
      const stage = container.querySelector<HTMLElement>(`[data-stage="${stageValue}"]`);
      if (stage) stage.scrollIntoView({ block: "nearest", inline: "start", behavior: "smooth" });
      else setStageFilter(stageValue);
    }
  };

  return (
    <div className="flex min-w-0 flex-col lg:h-full lg:min-h-[36rem]">
      <div className="flex flex-wrap items-center gap-3 px-3 sm:px-6 pt-4 sm:pt-6 pb-4 border-b border-border/40 shrink-0">
      ...
      {!isLoading && (
        <div className="px-3 sm:px-6 py-2.5 border-b border-border/30 shrink-0 flex flex-col lg:flex-row lg:items-center gap-3 bg-background/40">
          <div className="relative w-full lg:w-64 lg:shrink-0">
            ...
            <Input
              ...
              className="pl-8 pr-8 h-10 lg:h-8 text-sm lg:text-xs bg-background border-border/60"
            />
            {search && (
              <button
                onClick={() => setSearch("")}
                aria-label="Clear operations search"
      ...
      {!isLoading && (
        <div className="px-3 sm:px-6 py-2 border-b border-border/30 shrink-0 flex items-center gap-2 flex-wrap bg-secondary/10">
      ...
      {isLoading ? (
        ...
        <div data-testid="operations-board" className="lg:flex-1 lg:min-h-64 lg:overflow-x-auto" ref={scrollRef}>
          <div className="flex flex-col gap-6 p-3 sm:p-6 lg:flex-row lg:gap-4 lg:h-full lg:w-max lg:min-w-full">
      ...
      <div className="px-3 sm:px-6 py-3 border-t border-border/40 shrink-0 flex flex-wrap items-center gap-x-4 gap-y-2 text-[11px] text-muted-foreground">
        ...
        <span className="lg:ml-auto">Auto-refreshes every 60 seconds</span>
      </div>
    </div>
  );
}
