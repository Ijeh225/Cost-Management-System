...
| --- | --- | --- | --- |
| TD-01 | Two test branches and branch-scoped users | Branch A, Branch B, consolidated | Blocked | Only one branch currently exists. Two controlled branches must be created before branch isolation or comparison can be accepted. |
| TD-02 | Two clients and six containers across active/completed stages | Both branches | In progress | Existing controlled containers cover zero-value invoice, invoice collection, duty, and disbursement paths in one branch; cross-branch and full-stage coverage remains pending. |
...
| CONT-01 | Containers | Validate create, edit, duplicate protection, status progression, branch scope, and dashboard counters | Pending | |
