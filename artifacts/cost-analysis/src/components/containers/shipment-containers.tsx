...
export function ShipmentContainers({ containerId }: { containerId: number }) {
  ...
  return <JobOverviewPanel containerId={containerId}><section className="space-y-4 border-t border-border/60 pt-6">
    <h3 className="text-base font-semibold">Containers on this B/L</h3>
        ...
        <ul className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {data.containers.map(row => <li key={row.id} className="min-w-0">
            <Link href={`/containers/${row.id}`} aria-current={row.id === containerId ? "page" : undefined}
              className={`block rounded-xl border p-4 text-sm hover:bg-accent focus-visible:outline focus-visible:outline-2 break-words ${row.id === containerId ? "border-primary/30 bg-primary/5" : "border-border/60 bg-card"}`}>
