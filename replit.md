...
### Task #17 ✅ COMPLETE — Reports & print exports

Three printable report pages (registered outside AppLayout in `App.tsx`, inline styles only, `.no-print` for action buttons):

- **`/reports/client-statement/print`** — Client statement: invoice history with per-invoice payment rows, opening/closing balance, period filter (clientId, from, to query params). Component: `artifacts/cost-analysis/src/pages/reports/client-statement/print.tsx`.
- **`/reports/vat-summary/print`** — VAT summary for FIRS filing: subtotal/VAT/grand-total summary cards, per-invoice breakdown, optional from/to date filter. Component: `artifacts/cost-analysis/src/pages/reports/vat-summary/print.tsx`.
- **`/reports/invoice-aging/print`** — Live snapshot of all unpaid invoices in 5 overdue buckets (Current / 1–30 / 31–60 / 61–90 / 90+ days). Component: `artifacts/cost-analysis/src/pages/reports/invoice-aging/print.tsx`.

A **PrintableReportsSection** component was added at the bottom of the existing `/reports` page (below the container reports Tabs). It contains 3 cards that collect parameters (client dropdown + date pickers) and open the print pages in a new tab via `window.open()`.

Backend endpoints (admin-protected, in `artifacts/api-server/src/routes/reports.ts`):
- `GET /api/reports/client-statement?clientId=&from=&to=`
- `GET /api/reports/vat-summary?from=&to=`
- `GET /api/reports/invoice-aging`

API client hooks (in `lib/api-client-react/src/reports.ts`): `useGetClientStatement`, `useGetVatSummary`, `useGetInvoiceAging`.
