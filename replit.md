...
- Gate Security – Early Start Visibility: containers with `earlyStartAuthorized=true` appear in the gate-log API and on the Gate page as an "Expected Incoming" blue-tinted section; they are exempt from date-range filters so security staff can prepare for incoming containers.
- Documentation PAAR Smart Validation: "Save & Submit" in the Documentation workspace blocks unless both PAAR number and PAAR release date are present; a "Save Draft" button lets staff save partial progress without advancing the stage.
- Stage-specific workflow action labels (`OPS_RELEASE_LABELS`): Transire Out / Do Out / TDO Out / Confirm Pullout replace the generic "Mark as Released" in the Operations detail page; terminal workspace uses "Confirm Final Release".
- Security role access restrictions: ExternalLink buttons on the Gate page are hidden for `security_user` (admin-only); the container detail page renders a security-only restricted view (Container No, BL, Customer, Gate-In/Out) when accessed by a security user; notification clicks redirect security users to /gate instead of /containers/:id.
